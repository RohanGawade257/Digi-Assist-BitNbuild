...
export function VoiceAssistant({voice,locale,onEnd}:{voice:ReturnType<typeof useVoiceAssistant>;locale:Locale;onEnd:()=>void}){
 const text=copy[locale],d=designWords(locale),[settings,setSettings]=useState(false);
 return <section className="voice-assistant" aria-labelledby="voice-title" lang={locale} dir={locale==='ur-IN'?'rtl':'ltr'} data-voice-state={voice.state} data-interruption-ms={voice.interruptionMs}>
  <h2 id="voice-title">{text.title}</h2><p role="status" aria-live="polite">{voice.state==='idle'?text.start:text.states[voiceStates.indexOf(voice.state)]}</p>
  ...
  <button type="button" className="text-button voice-settings" onClick={()=>setSettings(true)}>{text.title} · {d.settings}</button>
  <Dialog open={settings} title={text.title+' · '+d.settings} closeLabel={d.close} onClose={()=>setSettings(false)}><label>{text.wait}<select value={voice.pauseMs} onChange={event=>voice.setPauseMs(Number(event.target.value))}><option value={2000}>2</option><option value={3500}>3.5</option><option value={5000}>5</option></select></label><p>{text.consent}</p><p>{text.commands}</p><p>{workspaceWords(locale).echo}</p></Dialog>
 </section>;
}
