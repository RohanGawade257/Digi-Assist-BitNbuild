...
export function VoiceAssistant({voice,locale,onEnd}:{voice:ReturnType<typeof useVoiceAssistant>;locale:Locale;onEnd:()=>void}){
 ...
 return <section className="voice-assistant" aria-labelledby="voice-title" lang={locale} dir={locale==='ur-IN'?'rtl':'ltr'} data-voice-state={voice.state}>
  ...
  <details><summary>{text.title}</summary><p>{text.consent}</p><p>{text.commands}</p></details>
 </section>;
}
