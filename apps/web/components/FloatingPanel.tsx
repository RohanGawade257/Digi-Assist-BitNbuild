...
export const FloatingPanel=forwardRef<FloatingHandle,Props>(function FloatingPanel({children,review,controls,open,locale,large,onOpen,onCloseChat,onClosedReturn,needsReview,question,error,caption,recovery,ended,onPause,onStopSpeaking,onEnd,onShare,sourceStatus,voiceState},ref){
 ...
 return <><div className="floating-toolbar">{controls}{supported?<button id="open-floating-assistant" type="button" className="primary" onClick={()=>void openWindow()}>{w.open}</button>:<p>{w.fallback}</p>}{notice&&<p role="status">{notice}</p>}</div><div ref={place}/>{host&&createPortal(
  <section className={`approval-workspace ${open?'chat-expanded':''} ${left?'review-left':''} ${opaque?'opaque':''} ${protectedView||touched?'protected':''}`} data-outside={outside} onPointerDown={()=>setTouched(true)} onFocusCapture={()=>setFocused(true)} onBlurCapture={event=>{if(!event.currentTarget.contains(event.relatedTarget as Node))setFocused(false);}}>
   <header className="workspace-header"><span className="workspace-source">{sourceStatus}<small role="status" data-state={voiceState}>{voiceState==='idle'?voiceCopy[locale].start:voiceCopy[locale].states[voiceStates.indexOf(voiceState)]}</small></span><button type="button" className="text-button" onClick={()=>setSettings(true)}>{d.settings}</button></header>
