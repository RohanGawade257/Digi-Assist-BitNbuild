...
export function Dialog({open,title,closeLabel,onClose,children}:{open:boolean;title:string;closeLabel:string;onClose:()=>void;children:ReactNode}){
 ...
 return <dialog ref={ref} className="app-dialog glass-surface" aria-labelledby={id} onCancel={event=>{event.preventDefault();callback.current();}}><header><h2 id={id}>{title}</h2><button type="button" className="secondary" onClick={onClose}>{closeLabel}</button></header><div className="dialog-content">{children}</div></dialog>;
}
