...
export const enabledLocales = ['en-IN', 'hi-IN', 'bn-IN', 'mr-IN', 'te-IN'] as const;
export const isEnabledLocale = (locale: string) => (enabledLocales as readonly string[]).includes(locale);
// Bulbul v3's documented output set excludes Urdu; do not bill unsupported requests.
export const speechOutputLocales = enabledLocales;
...
export const sourceSchema = z.object({
  ...
  userReviewed: z.boolean(), sanitizedHash: z.string().regex(/^[a-f0-9]{64}$/),
  captureId: z.uuid().optional(),
  screenConsent: z.object({ mode: z.literal('on-demand'), sourceId: z.uuid(), grantedAt: z.iso.datetime() }).strict().optional()
}).strict().superRefine((s, ctx) => {
  if(s.screenConsent ? (s.userReviewed || !s.captureId || !s.approvedImage || s.kind!=='screenshot' || s.contextMode!=='approved-image' || Date.parse(s.screenConsent.grantedAt)>Date.parse(s.capturedAt)) : !s.userReviewed) ctx.addIssue({code:'custom',message:'Explicit review or source-bound capture consent required'});
...
export const turnSchema = z.object({
  ...
  if(t.screenOverview ? (!t.source?.approvedImage || Boolean(t.question)) : !t.question) ctx.addIssue({code:'custom',message:'A question or image overview is required'});
  if ([t.inputLocale, t.replyLocale, t.draftLocale].some(locale => locale && !isEnabledLocale(locale))) ctx.addIssue({ code: 'custom', message: 'Language disabled for this version' });
...
export const preferencesSchema = z.object({
  replyLocale: localeSchema.refine(isEnabledLocale), interfaceLocale: localeSchema.refine(isEnabledLocale),
...
export const transcriptionSchema = z.object({ requestId: z.uuid(), inputLocale: localeSchema.refine(isEnabledLocale, 'Language disabled for this version'), cloudSpeechConsent: z.literal(true), automaticConversation: z.boolean().optional() }).strict();
