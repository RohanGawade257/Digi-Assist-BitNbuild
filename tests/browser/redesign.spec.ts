...
test('redesigned surfaces, native controller, keyboard dialogs and mobile navigation',async({page,context})=>{
 ...
 await page.screenshot({path:'test-results/vaanisetu-introduction.png',fullPage:true});
 await page.locator('.topbar').getByRole('button',{name:'Sign in',exact:true}).click();await expect(page.getByRole('dialog',{name:'VaaniSetu · Sign in',exact:true})).toBeVisible();await page.screenshot({path:'test-results/vaanisetu-authentication.png'});await page.keyboard.press('Escape');await expect(page.locator('.topbar').getByRole('button',{name:'Sign in',exact:true})).toBeFocused();
 ...
 await page.screenshot({path:'test-results/vaanisetu-workspace.png',fullPage:true});
 const a11y=await new AxeBuilder({page}).withTags(['wcag2a','wcag2aa']).analyze();expect(a11y.violations).toEqual([]);
 await page.locator('.topbar').getByRole('button',{name:'Accessibility',exact:true}).click();await page.screenshot({path:'test-results/vaanisetu-settings.png'});await page.keyboard.press('Escape');await expect(page.locator('.topbar').getByRole('button',{name:'Accessibility',exact:true})).toBeFocused();
 ...
 const opened=context.waitForEvent('page');await page.locator('#open-floating-assistant').click();const pip=await opened;await pip.setViewportSize({width:390,height:480});await expect(pip.getByRole('button',{name:'Share screen',exact:true})).toBeVisible();await pip.screenshot({path:'test-results/vaanisetu-floating-before.png'});
 await pip.getByRole('button',{name:'Share screen',exact:true}).click();const checkbox=pip.getByRole('checkbox',{name:/When I press Capture/});await expect(checkbox).toBeVisible();await pip.screenshot({path:'test-results/vaanisetu-floating-consent.png'});await checkbox.click();
 const primary=pip.getByRole('button',{name:'Capture current screen and send',exact:true});await expect(primary).toBeVisible();expect(await primary.evaluate(e=>e.getBoundingClientRect().bottom)).toBeLessThan(480);await pip.screenshot({path:'test-results/vaanisetu-floating-after.png'});
 await primary.click();await expect(pip.locator('.conversation-turn')).toHaveCount(1);await pip.getByRole('button',{name:'Chat',exact:true}).first().click();await pip.setViewportSize({width:800,height:650});await pip.locator('#question').fill('Keep my draft');await expect(pip.locator('.workspace-header')).toBeInViewport();await pip.screenshot({path:'test-results/vaanisetu-chat.png'});await pip.evaluate(()=>window.close());await expect(page.locator('#question')).toHaveValue('Keep my draft');await page.screenshot({path:'test-results/vaanisetu-sharing.png',fullPage:true});
 await page.setViewportSize({width:390,height:844});await page.locator('.workspace-tabs').getByRole('button',{name:'Conversation',exact:true}).click();await page.screenshot({path:'test-results/vaanisetu-mobile.png',fullPage:true});expect(await page.evaluate(()=>document.documentElement.scrollWidth)).toBeLessThanOrEqual(390);
