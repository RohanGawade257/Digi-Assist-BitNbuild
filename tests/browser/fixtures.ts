...
export async function login(page: Page) {
  ...
  await page.locator('.topbar').getByRole('button',{name:'Sign in',exact:true}).click();
  ...
  await page.locator('.auth-card').getByRole('button', { name: 'Sign in', exact: true }).click(); await expect(page.locator('.topbar').getByRole('button', { name: 'Account', exact: true })).toBeVisible();
