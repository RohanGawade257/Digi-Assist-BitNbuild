...
Updated: 11 September 2026 — consolidated product brief, revision 2.

This revision replaces the earlier overview and incorporates the subsequent scope, provider, authentication, capture, chat, and privacy decisions. It describes requirements, not features already implemented. The architecture kit is a separate implementation blueprint; neither document proves that the application has been built or tested.
...
The project is a general multilingual digital-task guide for Track 1: Access & Inclusion, specifically Inclusive Digital Services. It helps people use unfamiliar websites, understand instructions and errors, complete registrations, prepare email drafts, locate features or settings, and learn online workflows through spoken or typed questions and step-by-step guidance. Form filling is a major use case, not the product boundary.
...
2. Receive a draft in the selected language or an explicitly requested drafting language, with placeholders for excluded personal information.
...
3. Type a mixed-language question while viewing the target website.
...
Agents should implement this agreed behavior faithfully, expose limitations clearly, and avoid inventing capabilities or expanding the current scope without a concrete need. Detailed stack selection, architecture, schemas, endpoint contracts, and deployment instructions belong in Digital_Assistant_Architecture.zip. Read this updated overview for product behavior and the architecture kit for implementation planning. This revision supersedes the old overview; if a technical template cannot satisfy the stated experience or privacy requirements, surface the gap rather than silently changing the promise.
