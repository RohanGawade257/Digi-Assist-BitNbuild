...
async function main(){
 ...
 const localeArg=process.argv.indexOf('--locale');const languages=localeArg>=0?[process.argv[localeArg+1]]:['hi-IN','en-IN','bn-IN','mr-IN','te-IN','ta-IN'];
 try{for(const locale of languages){if(!copy[locale]||locale==='ur-IN')throw Error('Unsupported output language');
   fs.mkdirSync(path.join(target,locale),{recursive:true});
   for(const kind of ['intro','unclear','error','login','review','share','question','screenQuestion']){
